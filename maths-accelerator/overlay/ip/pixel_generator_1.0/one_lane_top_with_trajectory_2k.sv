// One lane of the 12-lane band-streaming pendulum solver
...
module one_lane_top #(
    ...
    parameter BAND_ROWS = 5      // image rows this lane buffers per band
)(
    input logic clk,
    input logic rst,
    input logic start,
    ...
    // resolution inputs (software)
    input logic [$clog2(IMG_W):0] img_w,
    input logic [$clog2(IMG_H):0] img_h,
    input logic [$clog2(IMG_W * IMG_H)-1:0] band_pix,
    ...
    // BRAM read port
    input  logic [$clog2(BAND_ROWS*IMG_W)-1:0] fb_rd_addr,
    ...
    // band handshake with the top-level readout FSM
    input  logic band_advance,   // 1-cycle pulse when overall band streamed
    output logic band_done // high once current lane done its portion responsible of overall band
);

    localparam BAND_H      = NUM_LANES * BAND_ROWS;   // image rows advanced per band
    localparam LOCAL_W     = $clog2(BAND_ROWS * IMG_W);
    localparam PIDW        = $clog2(IMG_W * IMG_H);

    logic [PIDW-1:0] band_total;
    assign band_total = band_pix * NUM_LANES;

    initial begin
        if (IMG_H % BAND_H != 0)
            $error("IMG_H (%0d) must be divisible by BAND_H = NUM_LANES*BAND_ROWS (%0d)", IMG_H, BAND_H);
    end

    // row_base = the top image row of this lane's sub-band 
    // band_id_base = the raster id of that row's first pixel
    // increment with band, saves DSP
    logic [$clog2(IMG_H)-1:0] row_base;
    logic [PIDW-1:0]          band_id_base;
    logic [$clog2(IMG_H)-1:0] q_top;       // q (row from image bottom) of the sub-band's top row

    assign q_top = img_h - 1 - row_base;

    // scanner walks current band
    logic [$clog2(IMG_W):0] scan_p;
    logic [$clog2(IMG_H):0] scan_q;
    logic [PIDW-1:0]          scan_id;
    logic scan_active;

    logic new_px_pending;
    logic new_px_valid;
    logic signed [W-1:0] new_px_x, new_px_y;
    logic [PIDW-1:0] new_px_id;

    logic coord_mapper_valid_out;
    logic coord_mapper_busy; // high while a coord occupies the 2-stage mapper pipeline

    logic [LOCAL_W:0] done_count;  // one bit wider so it counts up to BAND_PIX
    ...
    always_ff @(posedge clk) begin
        if (rst || !start) begin
            ...
            scan_p      <= '0;
            scan_q      <= '0;
            scan_id     <= LANE_ID * band_pix;
            row_base    <= LANE_ID * BAND_ROWS;
            band_id_base<= LANE_ID * band_pix;
            scan_active <= 1'b0;
        end
        else if (band_advance) begin
            // advance to next band, increment the accumulators and restart the scan
            row_base    <= row_base + BAND_H;
            band_id_base<= band_id_base + band_total;
            scan_id     <= band_id_base + band_total;
            scan_p      <= '0;
            scan_q      <= '0;
            scan_active <= 1'b0;
        end
            ...
            if (scan_p == img_w-1) begin
                scan_p <= '0;
                if (scan_q == BAND_ROWS-1) begin // last row of this band's sub-strip
                    scan_active <= 1'b0;
                end else begin
                    scan_q <= scan_q + 1;
                end
    ...
    // done_count tracks how many pixels written into frame buffer
    // reset on band_advance
    always_ff @(posedge clk) begin
        if (rst || !start)      done_count <= '0;
        else if (band_advance)  done_count <= '0;
        else if (fb_wr_en)      done_count <= done_count + 1;
    end
    assign band_done = (done_count >= band_pix); // lane done with its responsible part of band
